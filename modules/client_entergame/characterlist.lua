...
local function canClaimDailyReward(timestamp)
  if timestamp < 1 then
    return true
  end

  local currentTime = os.time()
  local tenOClock = os.time{year=os.date("%Y"), month=os.date("%m"), day=os.date("%d"), hour=10, min=0, sec=0}


  local latestDay = os.date("%d", timestamp)
  local currentDay = os.date("%d", os.time())

  if currentTime - timestamp > 24 * 60 * 60 then
    return true
  end

  if currentTime < tenOClock then
    return false
  end

  if latestDay == currentDay then
    return false
  end
...
function CharacterList.create(characters, account, otui)
    ...
    for i, characterInfo in ipairs(characters) do
        ...
        if g_game.getFeature(GameEnterGameShowAppearance) then
            ...
            if canClaimDailyReward(characterInfo.dailyreward) then
            ...
            if characterInfo.hidden == 1 then
                statusHidden:setImageSource('/images/game/entergame/hidden')
            else
                statusHidden:setImageSource('')
            end

            local characterLevel = widget:getChildById('level', characterList)

            if characterInfo.characterLevel then
              characterLevel:setText(characterInfo.characterLevel)
            end

            local characterVocation = widget:getChildById('vocation', characterList)

            if characterInfo.characterVocation then
              local vocName = tr("Guildless")

              local vocId = characterInfo.characterVocation
              if vocId == 1 then
                vocName = tr("Novice")
              elseif vocId == 2 then
                vocName = tr("Mercenary")
              elseif vocId == 3 then
                vocName = tr("Militia")
              elseif vocId == 4 then
                vocName = tr("Fire mage")
              elseif vocId == 5 then
                vocName = tr("Dragon Hunter")
              elseif vocId == 6 then
                vocName = tr("Paladin")
              end

              characterVocation:setText(vocName)
            end
        end
