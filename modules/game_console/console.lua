...
ChannelEventFormats = {
    [ChannelEvent.Join] = tr('%%s joined the channel.'),
    [ChannelEvent.Leave] = tr('%%s left the channel.'),
    [ChannelEvent.Invite] = tr('%%s has been invited to the channel.'),
    [ChannelEvent.Exclude] = tr('%%s has been removed from the channel.')
}
...
function removeTab(tab)
    ...
    elseif tab.channelId then
        if tab.channelId == 9 then
          setLootChannel(false)
        end
...
function setLootChannel(tof) lootChannel = tof end
    ...
    elseif speaktype.private then
        ...
        if channel then
          if channel == "Loot" then
            addText(composedMessage, SpeakTypesSettings.channelWhite, channel, name)
          else
            addText(composedMessage, speaktype, channel, name)
          end
        else
          if channelId == 9 then
            addText(composedMessage, SpeakTypesSettings.channelWhite, tr("Server Log"), name)
          else
...
local function getQuickLootBackpacks()
    ...
    for k, v in pairs(backpacks[char]) do
      data[k] = v
    end
...
--
local function sendQuickLootData()
  local stringList = ""
...
function online()
    ...
    scheduleEvent(function() ignoredChannels = { 9 } end, 3000)
end
...
function onChannelEvent(channelId, name, type)
    ...
    local channel = channels[channelId]
    if channel and channelId ~= 9 then
        local tab = getTab(channel)
