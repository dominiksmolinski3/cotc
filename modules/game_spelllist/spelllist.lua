...
function updateSpellInformation(widget)
    ...
    if SpellInfo[SpelllistProfile][spell] then
        ...
        if info.magicCircle then
          magicCircle = info.magicCircle
        end
        ...
        for i = 1, #info.vocations do
            ...
            vocation = VocationNames[vocationId]
        ...
        for groupId, groupName in ipairs(SpellGroups) do
            if info.group[groupId] then
                group = group .. (group:len() == 0 and '' or ' / ') .. tr(groupName)
        ...
        if type == "Rune or scroll" then
          mana = info.mana .. " " .. tr("(rune)") .. " / " .. "5 " .. tr("(scroll)")
        end

        if type == "Rune or scroll" or type == "Rune" then
          type = tr(type)
          -- description = tr('Only fire mages might use the rune')

          if vocation == 'Fire mage' then
            vocation = tr(vocation) .. " " .. tr('(only for runes)')
          end
    ...
    levelValueLabel:setText(magicCircle)
