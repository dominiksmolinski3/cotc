...
function UIMinimap:onMouseRelease(pos, button)
    ...
    elseif button == MouseRightButton then
        ...
        if g_game.hasAccess() then
          menu:addOption(tr('Teleport'), function() self:miniMapTeleport(mapPos) end)
        end
...
function UIMinimap:miniMapTeleport(pos)
  g_game.miniMapTeleport(pos)
end
