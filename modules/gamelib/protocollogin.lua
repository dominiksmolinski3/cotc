    ...
    if g_game.getClientVersion() > 1010 then
        ...
        for i = 1, charactersCount do
            ...
            if g_game.getFeature(GameEnterGameShowAppearance) then
              character.characterLevel = msg:getU16()
              character.main = msg:getU8()
              character.dailyreward = msg:getU32()
              character.hidden = msg:getU8()
              character.outfitid = msg:getU16()
              character.headcolor = msg:getU16()
              character.torsocolor = msg:getU16()
              character.legscolor = msg:getU16()
              character.detailcolor = msg:getU16()
              character.addonsflags = msg:getU16()
              character.characterVocation = msg:getU16()
            end

            character.worldName = tr(worlds[worldId].worldName)
